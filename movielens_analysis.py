...
class Links:
            ...
            if movie_id in self.imdb_links:
                ...
                try:
                    ...
                    for field in list_of_fields:
                        if field == 'Title':
                            title = soup.find('h1').find('span', {'data-testid': "hero__primary-text"})
                            movie_data.append(title.text.strip() if title else 'N/A')
    ...
    def get_tmdb(self, list_of_movies: list, list_of_fields: list):
        ...
        The method returns a list of lists [movieId, field1, field2, field3, ...] for the list of movies given as the argument (movieId).
        For example, [movieId, Director, Budget, Cumulative Worldwide Gross, Runtime].
        The values should be parsed from the TMDB webpages of the movies.
        Sort it by movieId descendingly.
        """
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
        tmdb_info = []
...
class Tests:
    ...
    def test_get_imdb(self, links_instance):
        links = links_instance
        movie_ids = list(links.movie_ids)[:10]
